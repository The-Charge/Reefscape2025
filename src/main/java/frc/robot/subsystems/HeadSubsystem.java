...
public class HeadSubsystem extends SubsystemBase {
  ...
  public void periodic() {
    ...
    else LoggingManager.logAndAutoSendValue("Head RunningCommand", getCurrentCommand().getName());
    ...
    LoggingManager.logAndAutoSendValue(
        "Head Branch Sensor LVL2 (Bool)", getBranchSensor(ElevSubsystem.Level.LVL2));
    LoggingManager.logAndAutoSendValue(
        "Head Branch Sensor LVL3 (Bool)", getBranchSensor(ElevSubsystem.Level.LVL3));
    LoggingManager.logAndAutoSendValue(
        "Head Branch Sensor LVL4 (Bool)", getBranchSensor(ElevSubsystem.Level.LVL4));
    ...
    switch (level) {
      ...
      case LVL3:
        return branchSensor.getRange() <= HeadConstants.branchLVL3ActivationDist
            && branchSensor.getRange() >= HeadConstants.branchLVL3MinActivationDist
            && branchSensor.getRangeSigma() < HeadConstants.branchSigmaActivation;
      case LVL2:
