...
public class ElevSubsystem extends SubsystemBase {
  ...
  public void periodic() {
    if (ElevConstants.hardStopResetsEncoder) checkHardStop();

    targetCheck();

    if (getCurrentCommand() == null)
      LoggingManager.logAndAutoSendValue("Elev RunningCommand", "None");
    else LoggingManager.logAndAutoSendValue("Elev RunningCommand", getCurrentCommand().getName());
    ...
    LoggingManager.logValue(
        "ElevPose",
        Pose3d.struct,
        new Pose3d(new Translation3d(0, 0, getPositionInches() / 39.37), Rotation3d.kZero),
        true);
    ...
    if (TelemetryConstants.debugTelemetry) {
      ...
      SmartDashboard.putNumber(
          "Elev Err (In)", (targetTicks - getPositionTicks()) * ElevConstants.tickToInConversion);
  ...
  public double getAppropriateL4Height() {
    if (DriverStation.isAutonomous()) return ElevConstants.lvl4InchesAuto;
  ...
  private void configureMotor(TalonFX m) {
    ...
    // motorConfig.HardwareLimitSwitch.ReverseLimitAutosetPositionEnable =
    // ElevConstants.hardStopResetsEncoder;
    // motorConfig.HardwareLimitSwitch.ReverseLimitAutosetPositionValue =
    // ElevConstants.hardStopResetValue;
  ...
  private Level getCurrentLevel(double inches) {
    if (inches
        <= ElevConstants.homeInches
            + ElevConstants.targetThresholdInches) // allow negatives to count as HOME
    return Level.HOME;
    ...
    else if (Math.abs(inches - ElevConstants.algaeHighInches)
        <= ElevConstants.targetThresholdInches) return Level.ALGAE_HIGH;
  ...
  private void targetCheck() {
    ...
    if (targetCounter >= ElevConstants.targetThresholdSeconds * 50) isAtTarget = true;
