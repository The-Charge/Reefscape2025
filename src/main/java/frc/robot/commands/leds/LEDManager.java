...
public class LEDManager extends Command {
  ...
  public LEDManager(
    ...
    breathe =
        new LEDDualBreatheAnimation(LEDConstants.chargeGreen, LEDConstants.chargeGold, 4, 0.2, 0.4);
    // steps = new LEDStepsAnimation(LEDConstants.chargeGreen, LEDConstants.chargeGold, 4,
    // Units.MetersPerSecond.of(Integer.MAX_VALUE));
    ...
    branchAligned =
        LEDPattern.solid(LEDConstants.chargeGreen)
            .blink(Units.Seconds.of(0.33), Units.Seconds.of(0.33));
  }
  ...
  public void execute() {
    ...
    if (DriverStation.isTeleopEnabled()) {
      if (Timer.getMatchTime() <= LEDConstants.endgameSecs
          && Timer.getMatchTime() >= LEDConstants.endgameSecs - LEDConstants.endgameDisplayTime) {
        ...
        if (Timer.getMatchTime() < LEDConstants.endgameSecs - LEDConstants.endgameRumbleTime
            && endgameStarted) {
    ...
    if (head.getHasCoral()) {
      if (LEDConstants.branchAlignLevels.contains(elev.getPositionLevel())
          && head.getBranchSensor(elev.getPositionLevel())) {
