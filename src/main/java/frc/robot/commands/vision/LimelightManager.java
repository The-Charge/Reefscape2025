...
public class LimelightManager extends Command {
  ...
  public LimelightManager(
      SwerveSubsystem swerve, VisionSubsystem reefLimelight, VisionSubsystem funnnelLimelight) {
    this.swerve = swerve;
  ...
  public void execute() {
    ...
    double yawRate =
        Math.abs(swerve.getSwerveDrive().getGyro().getYawAngularVelocity().in(DegreesPerSecond));
    double speed =
        Math.hypot(
    ...
    LimelightHelpers.PoseEstimate funnelEstimateTag1 =
        funnelLimelight.getLLHPoseEstimateTag1(yaw, 0);
    ...
    boolean reefEstim =
        (reefEstimate != null
            && reefCount > 0
            && yawRate < Units.degreesToRadians(60)); // reefAmbig < 0.2 ||
    boolean funnelEstim =
        (funnelEstimate != null
            && funnelCount > 0
            && yawRate < Units.degreesToRadians(60)); // funnelAmbig < 0.2 ||
    ...
    if ((reefAmbig < 0.2 || reefCount > 1) && yawRate < Units.degreesToRadians(36) && speed < 0.5) {
      swerve.addVisionReading(
          reefEstimateTag1.pose,
          reefEstimateTag1.timestampSeconds,
          VecBuilder.fill(9999999, 9999999, 2));
      LoggingManager.logAndAutoSendValue("Tag1 rotation", true);
    }

    if ((funnelAmbig < 0.2 || funnelCount > 1)
        && yawRate < Units.degreesToRadians(36)
        && speed < 0.5) {
      swerve.addVisionReading(
          funnelEstimateTag1.pose,
          funnelEstimateTag1.timestampSeconds,
          VecBuilder.fill(9999999, 9999999, 2));
      LoggingManager.logAndAutoSendValue("Tag1 rotation", true);
    }
    ...
    if (reefEstim && funnelEstim) {
      if (reefCount > funnelCount) {
        if (TelemetryConstants.debugTelemetry) SmartDashboard.putBoolean("reef estimated", true);

        swerve.addVisionReading(
            reefEstimate.pose, reefEstimate.timestampSeconds, VecBuilder.fill(.7, .7, 9999999));
      } else if (reefCount < funnelCount) {
        if (TelemetryConstants.debugTelemetry) SmartDashboard.putBoolean("funnel estimated", true);

        swerve.addVisionReading(
            funnelEstimate.pose, funnelEstimate.timestampSeconds, VecBuilder.fill(.7, .7, 9999999));
