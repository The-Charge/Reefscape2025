...
public class DriveToAlgae extends Command {
  ...
  public void execute() {
    ...
    double RotationVal =
        MathUtil.clamp(heading_controller.calculate(limelight.getTX(), 0.0), -1, 1);
    double TranslationVal =
        MathUtil.clamp(drive_controller.calculate(limelight.getDistToCamera(), 0.0), -0.5, 0.5);
    ...
    if (limelight.getObjectedDetected()) {
      ...
      swerve.drive(
          new Translation2d(-1 * TranslationVal * SwerveConstants.MAX_SPEED, 0),
          RotationVal * swerve.getSwerveController().config.maxAngularVelocity,
          false);
    }
