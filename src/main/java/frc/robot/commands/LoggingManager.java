...
public abstract class LoggingManager {
  ...
  private static final HashMap<String, StructLogEntry>
      structEntries; // use raw types because generics suck
  ...
  /**
   * Logs a value to the on-robot log file. Note that the key automatically has "/AvScope/" appended
   * to the beginning
   */
  ...
  public static void logValue(String key, boolean val, boolean ignoreRepeats) {
    ...
    if (!boolEntries.containsKey(newKey)) {
      ...
      entry.update(
          val); // no chance of repeat value but still need to use update rather than append in
      // order to update last value
  ...
  public static <T> void logValue(String key, Struct<T> struct, T[] vals, boolean ignoreRepeats) {
    ...
    if (!structArrayEntries.containsKey(newKey)) {
      StructArrayLogEntry<T> entry =
          StructArrayLogEntry.create(DataLogManager.getLog(), newKey, struct);
      entry.update(
          vals); // no chance of repeat value but still need to use update rather than append in
      // order to update last value
  ...
  /**
   * Logs a value to the on-robot log file and sends it to the network tables if {@link
   * frc.robot.constants.TelemetryConstants#debugTelemetry TelemetryConstants.debugTelemetry} is
   * true
   */
