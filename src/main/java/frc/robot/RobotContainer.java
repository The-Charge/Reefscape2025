...
public class RobotContainer {
  ...
  private final VisionSubsystem reeflimelight =
      new VisionSubsystem(LLReefConstants.LL_NAME, LLReefConstants.CAMERA_OFFSET, true);
  private final VisionSubsystem funnellimelight =
      new VisionSubsystem(LLFunnelConstants.LL_NAME, LLFunnelConstants.CAMERA_OFFSET, true);
  ...
  public RobotContainer() {
    ...
    hid1 =
        driver1.getHID(); // use hid objects to reduce performance impact. Using getBoolean() on the
    // trigger from CommandXboxController causes large CPU usage
    hid2 = driver2.getHID();
        ...
        new TeleopDrive(
            ...
            () ->
                MathUtil.applyDeadband(
                    hid1.getRightTriggerAxis(), SwerveConstants.TRIGGER_DEADBAND));
  ...
  private void configureBindings() {
    ...
    driver2
        .leftTrigger(SwerveConstants.TRIGGER_DEADBAND)
        .onTrue(new MoveToLevel(elev, head, Level.HOME));
    ...
    new Trigger(
            () ->
                hid2.getRightY() < -SwerveConstants.RIGHT_Y_DEADBAND
                    && elev.getPositionLevel() == ElevSubsystem.Level.LVL4)
        .whileTrue(new AdjustL4Override(elev, 0.025));
    new Trigger(
            () ->
                hid2.getRightY() > SwerveConstants.RIGHT_Y_DEADBAND
                    && elev.getPositionLevel() == ElevSubsystem.Level.LVL4)
    ...
    driver2
        .start()
        .whileTrue(
            new ElevManualUp(elev).withInterruptBehavior(InterruptionBehavior.kCancelIncoming));
  }
  ...
  private void configureNamedCommands() {
    ...
    NamedCommands.registerCommand(
        "ElevHome", new MoveToLevel(elev, head, ElevSubsystem.Level.HOME, true));
    NamedCommands.registerCommand(
        "ElevLevel1", new MoveToLevel(elev, head, ElevSubsystem.Level.LVL1, true));
    NamedCommands.registerCommand(
        "ElevLevel2", new MoveToLevel(elev, head, ElevSubsystem.Level.LVL2, true));
    NamedCommands.registerCommand(
        "ElevLevel3", new MoveToLevel(elev, head, ElevSubsystem.Level.LVL3, true));
    NamedCommands.registerCommand(
        "ElevLevel4", new MoveToLevel(elev, head, ElevSubsystem.Level.LVL4, true));
    NamedCommands.registerCommand(
        "ElevAlgaeLow", new MoveToLevel(elev, head, ElevSubsystem.Level.ALGAE_LOW, true));
    NamedCommands.registerCommand(
        "ElevAlgaeHigh", new MoveToLevel(elev, head, ElevSubsystem.Level.ALGAE_HIGH, true));
  ...
  private void addTelemetry() {
    ...
    if (TelemetryConstants.debugTelemetry) {
      ...
      SmartDashboard.putData(
          "Elev Zero", new InstantCommand(elev::setAsZero).ignoringDisable(true));
  ...
  private void setupAutoDisplay() {
    ...
    else LoggingManager.logValue("SelectedAuto", "Null");
  ...
  public void scheduleLimelightAuton() {
    if (SwerveConstants.autonVisionTime == 0) return;
