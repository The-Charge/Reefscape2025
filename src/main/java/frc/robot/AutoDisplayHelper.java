...
public abstract class AutoDisplayHelper {
  ...
  public static void displayAutoPath(Command autoCommand, boolean isRedAlliance) {
    ...
    if (autoCommand == null || autoCommand.getName().equals("InstantCommand")) {
      DriverStation.reportError(
          "Cannot display the path of a null/empty command, use clearAutoPath instead", true);
      return;
    }
    ...
    if (mergedTraj == null) {
      DriverStation.reportWarning(
          "AutoDisplayHelper::displayAutoPath -> Auto has no paths, redirecting to clearAutoPath",
          false);
      clearAutoPath();
      return;
    }
  ...
  private static Trajectory generateTrajectory(List<Pose2d> poses) {
    TrajectoryConfig trajConfig =
        new TrajectoryConfig(3, 3); // values don't matter as long as they aren't zero
  ...
  private static Rotation2d calculatePoseRotation(PathPoint current, PathPoint next) {
    ...
    double rad =
        Math.atan2(
            next.position.getY() - current.position.getY(),
            next.position.getX() - current.position.getX());
    ...
    for (int i = 0; i < pathPoints.size(); i++) {
      ...
      if (i == 0) {
        if (pathPoints.size() > 1)
          rot =
              calculatePoseRotation(
                  pp,
                  pathPoints.get(
                      i + 1)); // For the first point, use the heading towards the second point
        else rot = new Rotation2d(); // If there's only one point, use a default rotation
      } else if (i == pathPoints.size() - 1)
        rot =
            calculatePoseRotation(
                pathPoints.get(i - 1),
                pp); // For the last point, use the heading from the second-to-last point
      else {
        // For intermediate points, calculate the heading as the average of the direction to the
